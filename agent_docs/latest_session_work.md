...
- `session-capture --start-on-a-release` waits on a read-only physical XInput slot-0 A hold and begins after the release edge.
- `--progress` writes an elapsed/remaining timer to stderr while preserving final JSON on stdout.
- The physical 8BitDo path successfully armed and started both live captures.
...
- Final full suite: 79 tests passed; Ruff, Mypy, `uv lock --check`, and `git diff --check` passed.
...
- Accepted solo corpus: approximately 14.53 minutes of the 120–240 minute target.
- Record at least seven more accepted approximately 15-minute sessions for the two-hour minimum; prefer nine more so ten whole sessions provide stronger deterministic 80/10/10 split granularity.
