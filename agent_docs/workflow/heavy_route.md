...
Every worker spawn and the initial creation of the explorer companion must use Pi's subagent workflow API with an explicit context choice and the appropriate configured agent. The initial task capsule must be self-contained and at most 400 words. Use these fixed fields: task ID, outcome, ownership, acceptance criteria, source paths, validation, protected areas, and return format. Include only the minimum initial context grouped as:
...
Do not repeat this stable batching policy in task capsules or follow-up deltas. Capsules define package-specific scope and evidence; the configured Pi agent definitions define persistent role behavior.
