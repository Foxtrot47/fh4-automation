...
- XInput player slots are runtime attachment indices, not stable controller identities. Capture therefore auto-selects only when exactly one slot is connected, fails closed on ambiguity, allows an explicit 0–3 override, uses the resolved slot consistently for gating and sampling, and records it in session metadata. XInput state alone does not prove the controller model, so model provenance remains operator-supplied evidence.
...
- The second accepted Rivals session is one uninterrupted 15-minute segment with 26,979 aligned frames, no alignment or continuity failures, no recorded button presses, and no visible car or ghost in four representative frames. Together the first two sessions provide 53,138 aligned frames, approximately 29.52 frame-equivalent minutes.
