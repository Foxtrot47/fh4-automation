...
**Fixed capture profile:** Borderless 2560×1440 game output; stored 960×540 JPEG frames at 30 FPS; physical control sampled through read-only XInput with unique-slot auto-selection or an explicit 0–3 override; exact FH4 UDP datagrams retained. The accepted FH4-200 hardware artifact used the 8BitDo Ultimate 2C at slot 0.
...
**Status:** In progress — offline tooling accepted; five solo sessions provide approximately 74.27 validated frame-equivalent minutes.
...
**Software evidence:** FH4-310 through FH4-340 are implemented. Strict streaming validation binds source manifests and exact binary/sidecar telemetry, rejects fatal health/config/schema/continuity faults, reports independent per-stream rates and bounded alignment distributions, and performs 1% bounded frame salvage. Lap candidates exclude partial/reset laps and tolerate duplicates/uint32 wrap without claiming collision-free status. Deterministic whole-session splits, safe tar names, 1,024-sample shard bounds, model-safe ego/slip/pose features, checksummed shards/reports, CLI validation/build, and no-overwrite behavior are covered. Live collection added an explicit read-only XInput A hold/release start gate and stderr progress timer. Controller discovery now auto-selects exactly one connected XInput slot, fails closed on zero or multiple devices unless an explicit slot is supplied, and records the resolved slot in backward-compatible session metadata. Dataset validation preserves all substantial race-on segments across pause boundaries, resets continuity/lap state per segment, excludes short transient and pause/post-race frames, caps segments and lap candidates, and keeps open segments bounded by the final telemetry time plus the 33.334 ms alignment allowance. Current local evidence: 80 full tests, Ruff, Mypy, lock check, package build, fresh-wheel discovery smoke, a complete live capture with `controller_slot=0`, and diff check passed. The accepted FH4-200 artifact still validates as 341 aligned frames with 29.9718/125.0834/164.9700 Hz frame/controller/telemetry rates, p95 alignment 0/0 ms, max 16/0 ms, and zero estimated gaps/out-of-order events. A real build produced a 341-sample tar whose SHA-256 matched its manifest and excluded opaque/applied-control fields.
**Verification:** The original independent review found three blockers (insufficient safe telemetry features, missing alignment distributions, and uint32-wrap handling); all were corrected and tested. Live collection then exposed post-race and pause segmentation defects. A fresh independent review found two further boundedness issues—unbounded lap candidates and open segments absorbing non-telemetry tails—which were corrected with adversarial tests before publication. Final independent code rereview passed with no blockers, and independent artifact review approved Rivals solo session 001 for corpus inclusion subject to recorded provenance/visual-sampling caveats. A later independent review passed configurable XInput selection; requested explicit-slot and legacy/new manifest integration assertions were added and passed.
**Collection evidence:** Accepted solo artifact `recordings/fh4-300-rivals-solo-001-20260811-181528` (session `e4a61b8b-437e-4485-ba88-2f1b305bfa7b`) contributes 26,159 aligned frames across three pause-separated race segments, approximately 14.53 usable minutes, three isolated alignment rejections, zero selected-segment missing/out-of-order timestamps, and 16 candidate complete laps with a 44.754-second median. Accepted artifact `recordings/fh4-300-rivals-solo-002-20260811-205759` (session `ac6a88e8-65bb-4f97-bd12-81bb39cdf242`) contributes one continuous 15-minute segment, 26,979 aligned frames, zero alignment rejections/exclusions, and 19 candidate laps with a 44.145-second median. Accepted artifact `recordings/fh4-300-rivals-solo-003-20260811-213220` (session `1cac205d-c50f-45bd-8d5c-bfc1dafe6d1d`) contributes another continuous 15-minute segment, 26,981 aligned frames, zero alignment rejections/exclusions, and 19 candidate laps with a 44.024-second median. Accepted artifact `recordings/fh4-300-rivals-solo-004-20260811-224414` (session `dae12572-c3d5-4064-bfa3-5993ba6ace76`) contributes 26,589 frames from two pause-separated segments, one isolated alignment rejection, 389 excluded non-race frames, and 18 candidate laps with a 44.091-second median. Accepted artifact `recordings/fh4-300-rivals-solo-005-20260811-230419` (session `d481aa03-b57b-489e-8c01-2bd89ea60d83`) adds one continuous segment with 26,979 aligned frames, zero alignment rejections/exclusions, and 19 candidate laps with a 44.017-second median. All five sessions have zero selected-race missing/out-of-order timestamps and no observed rewind input; representative race frames showed no cars or visible ghost. Combined accepted coverage is 133,687 frames, approximately 74.27 frame-equivalent minutes. The earlier 20-lap Drivatar artifact validates the pipeline but is excluded from the solo corpus.
**Gate:** No session leakage across splits; every sample carries session/benchmark/config identity; corrupt or incompatible inputs fail closed; frame/controller/telemetry alignment uses integer `session_ns` with a 33.334 ms default bound; reports expose stream rates, controls, health, alignment, accepted/rejected samples, telemetry continuity, and candidate complete-lap coverage. Shard output is deterministic, checksummed, bounded-memory, and refuses to overwrite existing output. Full FH4-300 acceptance still requires the planned 2–4 hours of user demonstrations, held-out split coverage, an immutable dataset build, and a human baseline report.
...
- FH4-300 has five accepted Rivals solo sessions (approximately 74.27 frame-equivalent minutes) and still requires approximately 45.73 minutes for the two-hour minimum plus enough whole sessions for held-out split/baseline coverage.
...
Record Rivals solo session 006 for 900 seconds with the A-release gate, no per-second progress output, and a non-injecting three-beep completion signal. Validate it immediately and inspect representative frames. Continue to at least nine accepted approximately 15-minute solo sessions total for the two-hour minimum; prefer ten or more for stronger 80/10/10 split granularity. Keep clean-lap and deliberate safe-recovery sessions identifiable, do not use rewind, and exclude traffic sessions from the solo corpus. Build the immutable dataset and human baseline report only after collection and independent acceptance. Azure quota remains independent of local dataset work.
