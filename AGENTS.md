...
For each bounded work stage, identify independent, already-known, non-conflicting tool calls before invoking tools. When practical, batch them with Pi's parallel tool facilities.
...
This workflow runs in Pi. Use the `functions.subagent` workflow API for orchestration, `runs.run` for one child, and `runs.all` for independent parallel child lanes. Use the configured agents `explorer`, `executor_luna`, `executor_sol`, `tester`, and `doc-writer` by name. Keep one writer per working directory, prefer fresh-context review, and use `functions.subagent_wait` only when the current task must finish in the same turn.
